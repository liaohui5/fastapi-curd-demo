...
class Curd:
    ...
    async def create(self, data: dict):
        result = await self.session.exec(self.model.insert().values(**data))
    ...
    async def hard_delete(self, id: int):
        result = await self.session.exec(self.model.delete().where(self.model.id == id))
        await self.session.commit()
        return result.rowcount

    async def detail(self, id: int):
        result = await self.session.exec(self.model.select().where(self.model.id == id))
        return result.one()
